...
C$OPENAD XXX Template ad_template.f
      ...
      CALL sax(OpenAD_lin_1,OpenAD_prp_0,MATR(1))
      MATR(2)%v = (X(5)%v-X(4)%v)
      CALL setderiv(MATR(2),X(5))
      CALL dec_deriv(MATR(2),X(4))
      OpenAD_lin_2 = (X(6)%v*2.0D00-X(5)%v-X(4)%v)
      MATR(3)%v = (SQRT3*OpenAD_lin_2)
      OpenAD_lin_3 = SQRT3
      CALL set_neg_deriv(OpenAD_prp_1,X(4))
      CALL dec_deriv(OpenAD_prp_1,X(5))
      CALL saxpy(2.0D00,X(6),OpenAD_prp_1)
      CALL sax(OpenAD_lin_3,OpenAD_prp_1,MATR(3))
      M03%v = (MATR(0)%v*MATR(3)%v)
      OpenAD_lin_4 = MATR(3)%v
      OpenAD_lin_5 = MATR(0)%v
      M12%v = (MATR(1)%v*MATR(2)%v)
      OpenAD_lin_6 = MATR(2)%v
      OpenAD_lin_7 = MATR(1)%v
      G%v = (M03%v-M12%v)
      D00 = (M03%v*2.0D00)
      D11 = (M12%v*2.0D00)
      M00%v = (MATR(0)%v*MATR(0)%v)
      OpenAD_lin_8 = MATR(0)%v
      OpenAD_lin_9 = MATR(0)%v
      M11%v = (MATR(1)%v*MATR(1)%v)
      OpenAD_lin_10 = MATR(1)%v
      OpenAD_lin_11 = MATR(1)%v
      M22%v = (MATR(2)%v*MATR(2)%v)
      OpenAD_lin_12 = MATR(2)%v
      OpenAD_lin_13 = MATR(2)%v
      M33%v = (MATR(3)%v*MATR(3)%v)
      OpenAD_lin_14 = MATR(3)%v
      OpenAD_lin_15 = MATR(3)%v
      D12%v = (M00%v*M33%v)
      OpenAD_lin_16 = M33%v
      OpenAD_lin_17 = M00%v
      D03%v = (M11%v*M22%v)
