...
C$OPENAD XXX Template ad_template.f
      ...
      DO WHILE(I .LT. 3)
        IF(I .LT. 2) THEN
          ...
          CALL sax(OpenAD_lin_0,OpenAD_prp_0,Y(2))
        ELSE
          OpenAD_dly_1 = COS(X(2)%v)
          OpenAD_lin_1 = (-SIN(X(2)%v))
          Y(1)%v = OpenAD_dly_1
          CALL setderiv(OpenAD_prp_1,X(2))
          CALL sax(OpenAD_lin_1,OpenAD_prp_1,Y(1))
        ENDIF
        I = (I+1)
      END DO
      ...
      CALL setderiv(OpenAD_prp_3,Y(2))
      CALL sax(OpenAD_lin_2,OpenAD_prp_2,Y(2))
      CALL saxpy(OpenAD_lin_3,OpenAD_prp_3,Y(2))
      END SUBROUTINE
